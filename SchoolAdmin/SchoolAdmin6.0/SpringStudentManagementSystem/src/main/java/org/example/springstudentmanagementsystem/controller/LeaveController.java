...
public class LeaveController {
    ...
    public Map<String, Object> approveLeave(
        ...
        try {
            ...
            approval.setApproverRole(approver.getRole());  // 从session中获取用户角色
    ...
    @GetMapping("/draft/{id}")
    @ResponseBody
    public Leave getDraft(@PathVariable String id, HttpSession session) {
        String studentId = ((User) session.getAttribute("user")).getId();
        Leave leave = leaveService.findLeaveById(id);
        if (leave != null && leave.getStudentId().equals(studentId) && "DRAFT".equals(leave.getStatus())) {
            return leave;
        }
        return null;
    }
